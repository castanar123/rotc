...
<div class="dashboard-sidebar">
    <ul class="sidebar-menu">
        <?php foreach ($menu_items as $key => $item): ?>
                ...
                <li class="nav-item-dropdown <?php echo $is_active ? 'open' : ''; ?>">
                    ...
                    <ul class="dropdown-menu" style="display: <?php echo $is_active ? 'block' : 'none'; ?>;">
                        <?php foreach ($item['sub_items'] as $sub_url => $sub_text): ?>
                            <li>
                                <a href="/generate%20qr/<?php echo $key . '/' . $sub_url; ?>" class="<?php echo ($current_page == $sub_url) ? 'active-sub' : ''; ?>">
            ...
            <?php else: ?>
                 <li>
                    <a href="/generate%20qr/<?php echo $key; ?>" class="<?php echo ($current_page == $key) ? 'active' : ''; ?>">
                        <i class="fas <?php echo $item['icon']; ?>"></i>
                        <span><?php echo $item['text']; ?></span>
                    </a>
                </li>
