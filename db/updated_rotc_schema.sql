...
-- Add email field to cadet_profiles table if it doesn't exist
ALTER TABLE `cadet_profiles` 
ADD COLUMN IF NOT EXISTS `email` varchar(255) DEFAULT NULL AFTER `full_name`;

-- Update status enum to include 'pending' option
ALTER TABLE `cadet_profiles` 
MODIFY COLUMN `status` enum('pending','Active','Inactive','Graduated') NOT NULL DEFAULT 'pending';

-- Update users table status to include 'pending'
ALTER TABLE `users` 
...
('current_semester', '1st Semester', 'string', 'Current academic semester'),
('current_academic_year', '2024-2025', 'string', 'Current academic year'),
('attendance_weight', '30', 'number', 'Attendance component weight in percentage'),
('quiz_weight', '25', 'number', 'Quiz component weight in percentage'),
('participation_weight', '25', 'number', 'Participation component weight in percentage'),
('conduct_weight', '20', 'number', 'Conduct component weight in percentage'),
('qr_expiration_minutes', '30', 'number', 'QR code expiration time in minutes'),
('max_file_upload_size', '10485760', 'number', 'Maximum file upload size in bytes (10MB)'),
('allowed_file_types', '["pdf","doc","docx","jpg","jpeg","png"]', 'json', 'Allowed file types for uploads');
