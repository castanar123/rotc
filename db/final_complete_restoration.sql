-- Final Complete ROTC Database Restoration Script
...
CREATE TABLE IF NOT EXISTS `borrower_temp_ids` (
  ...
  `cadet_number` varchar(20) DEFAULT NULL,
  `created_at` datetime DEFAULT CURRENT_TIMESTAMP,
  `expires_at` datetime DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `attendance_records` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `cadet_number` varchar(20) NOT NULL,
  `td` int(11) NOT NULL,
  `semester` int(11) NOT NULL,
...
-- Cadet profiles indexes (using existing cadet_id column)
CREATE INDEX IF NOT EXISTS `idx_cadet_profiles_cadet_id` ON `cadet_profiles`(`cadet_id`);

-- Attendance indexes
CREATE INDEX IF NOT EXISTS `idx_attendance_date` ON `attendance`(`date`);
CREATE INDEX IF NOT EXISTS `idx_attendance_cadet_date` ON `attendance`(`cadet_id`, `date`);
...
SELECT 
  cp.id,
  cp.cadet_id as student_id,
  CONCAT(IFNULL(cp.company, ''), ' ', IFNULL(cp.platoon, ''), ' ', IFNULL(cp.squad, '')) as name,
  NOW() as created_at
...
SELECT 'Final Complete ROTC database restoration completed successfully!' as message;
