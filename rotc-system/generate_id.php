...
// --- Build ROTC_QR_V1 cadet payload for ID card QR ---
$student_id = $profile['student_id'] ?? '';
$last_name_field = $profile['last_name'] ?? $last_name;
$platoon_value = $profile['platoon'] ?? 'DELTA SECOND';

$cadet_qr_payload = [
    'system' => 'rotc_system',
    'type' => 'cadet',
    'profile_id' => 'CDT-' . (string)$profile_id,
    'student_id' => (string)$student_id,
    'last_name' => strtoupper($last_name_field),
    'platoon' => $platoon_value,
];

$cadet_qr_json = json_encode($cadet_qr_payload);
$cadet_qr_encoded = '';
if ($cadet_qr_json !== false) {
    $cadet_qr_encoded = base64_encode('ROTC_QR_V1::' . $cadet_qr_json);
}
...
<script type="text/javascript">
    ...
    // The data to encode (ROTC_QR_V1 cadet payload, base64-encoded)
    var dataToEncode = "<?php echo htmlspecialchars($cadet_qr_encoded, ENT_QUOTES, 'UTF-8'); ?>";
